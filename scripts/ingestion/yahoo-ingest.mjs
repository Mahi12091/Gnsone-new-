...
const yahoo = async (url) => {
  const response = await fetch(url, { headers: { 'User-Agent': 'GNSOne/1.0 research-ingestion' } });
  if (!response.ok) throw new Error(`Yahoo ${response.status}: ${url}`);
  return response.json();
};

const supabaseRpc = async (fn, body) => {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${fn}`, { method: 'POST', headers, body: JSON.stringify(body) });
...
function valuesByType(map, type) {
  ...
  return rows.map(x => ({ date: new Date((x.asOfDate || 0) * 1000).toISOString().slice(0, 10), value: num(x.reportedValue?.raw ?? x.reportedValue) })).filter(x => x.value != null).sort((a, b) => a.date.localeCompare(b.date));
}
...
async function ingestStock(stock) {
  ...
  const prices = timestamps.map((ts, i) => ({
    ...
    open: num(q.open?.[i]), high: num(q.high?.[i]), low: num(q.low?.[i]), close: num(q.close?.[i]), adj_close: num(adj[i] ?? q.close?.[i]), volume: num(q.volume?.[i]),
  ...
  const ratios = [
    ['ROE', roe], ['ROA', roa], ['DEBT_TO_EQUITY', debtEquity], ['NET_MARGIN', netMargin], ['EBITDA_MARGIN', ebitdaMargin], ['REVENUE_GROWTH_1Y', revenueGrowth], ['PROFIT_GROWTH_1Y', profitGrowth], ['EPS_GROWTH_1Y', epsGrowth],
  ].filter(([,v]) => v != null).map(([code,value]) => ({ code, value, period_type:'ANNUAL', period_end: series.revenue.at(-1)?.date || asOf }));
