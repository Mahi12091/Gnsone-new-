...
async function ingestYahooActions(stock) {
  ...
  for (const [ts,item] of Object.entries(events.dividends || {})) {
    const ex=date(Number(ts)), amount=number(item?.amount); if(!ex||amount==null) continue;
  ...
  if(splitType) for(const [ts,item] of Object.entries(events.splits||{})){
    const ex=date(Number(ts)), split=String(item?.numerator??''), [n,d]=split.split(':').map(Number);
...
async function ingestNseShareholding(stock,categories){
  let payload; try{payload=await nse(`/api/corporate-share-holdings-master?index=equities&symbol=${encodeURIComponent(stock.nse_symbol)}`);}catch{return false;}
  const rows=Array.isArray(payload)?payload:(payload?.data||payload?.shareholding||payload?.result||[]); if(!rows.length)return false;
  const latest=rows.at(-1), periodEnd=date(latest?.date??latest?.periodEndDate??latest?.period_end_date??latest?.quarterEndDate); if(!periodEnd)return false;
  ...
  if(!periodId){const inserted=await sb('market.shareholding_periods',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({company_id:stock.company_id,period_end_date:periodEnd,period_type:'QUARTERLY',filing_date:date(latest?.filingDate),source_id:NSE_SOURCE})});periodId=inserted[0]?.shareholding_period_id;}
  ...
  for(const row of rows){const raw=norm(row?.category??row?.categoryName??row?.shareholderCategory??row?.type),match=Object.keys(aliases).find(k=>raw===k||raw.includes(k)),categoryId=match?categories[aliases[match]]:null,pct=number(row?.percentage??row?.percentageHolding??row?.percentShareholding??row?.shareholdingPercentage),shares=number(row?.shares??row?.sharesHeld??row?.noOfShares);if(!categoryId||(pct==null&&shares==null))continue;await sb('market.shareholding_values',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({shareholding_period_id:periodId,shareholding_category_id:categoryId,shares_held:shares,percentage_held:pct,source_id:NSE_SOURCE})});}
  return true;
}

async function ingestPeers(stocks){
  const companies=await sb('market.companies?select=company_id,industry_id,sector_id'),meta=new Map(companies.map(c=>[c.company_id,c])),groups=new Map();
  for(const s of stocks){const c=meta.get(s.company_id),key=c?.industry_id||c?.sector_id;if(!key)continue;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(s);}
  for(const members of groups.values())for(const company of members)for(const peer of members){if(company.company_id===peer.company_id)continue;await sb('market.company_peers',{method:'POST',headers:{Prefer:'resolution=ignore-duplicates,return=minimal'},body:JSON.stringify({company_id:company.company_id,peer_company_id:peer.company_id,relationship_type:'INDUSTRY_PEER',source_id:YAHOO_SOURCE})});}
}
...
async function main(){
  ...
  let actionSymbols=0,holdingSymbols=0;
  for(const stock of stocks){try{await ingestYahooActions(stock);actionSymbols++;}catch(e){console.error(`actions ${stock.nse_symbol}: ${e.message}`);}try{if(await ingestNseShareholding(stock,categories))holdingSymbols++;}catch(e){console.error(`holdings ${stock.nse_symbol}: ${e.message}`);}}
  try{await ingestPeers(stocks);}catch(e){console.error(`peers: ${e.message}`);}
  console.log(JSON.stringify({instruments:stocks.length,action_symbols:actionSymbols,shareholding_symbols:holdingSymbols,peer_mode:'industry-derived',shareholding_source:'NSE_PUBLIC'}));
}
