...
export async function getStockSnapshots(limit = 6): Promise<StockDetail[]> {
  ...
  return stocks.map((stock) => ({
    ...stock,
    latest_price: stock.nse_listing_id ? latestPrice.get(stock.nse_listing_id)?.close ?? null : null,
    price_date: stock.nse_listing_id ? latestPrice.get(stock.nse_listing_id)?.price_date ?? null : null,
