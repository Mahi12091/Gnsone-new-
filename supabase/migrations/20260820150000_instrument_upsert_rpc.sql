...
declare
  ...
  v_action text;
  v_identifier_type identifier_type;
...
begin
  ...
  -- Stable identifiers always win. Multiple matches are treated as a hard conflict.
  if v_isin is not null then
    select count(*), min(id) into v_match_count, v_existing_id
      from public.instruments where isin = v_isin;
    if v_match_count > 1 then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: multiple instruments share ISIN %', v_isin;
    end if;
  ...
  if v_instrument_id is null and v_nse_symbol is not null then
    select count(*), min(id) into v_match_count, v_existing_id
      from public.instruments where nse_symbol = v_nse_symbol;
    if v_match_count > 1 then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: multiple instruments share NSE symbol %', v_nse_symbol;
    end if;
    if v_match_count = 1 then v_instrument_id := v_existing_id; end if;
  end if;
  ...
  if v_instrument_id is null and v_bse_code is not null then
    select count(*), min(id) into v_match_count, v_existing_id
      from public.instruments where bse_code = v_bse_code;
    if v_match_count > 1 then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: multiple instruments share BSE code %', v_bse_code;
    end if;
    if v_match_count = 1 then v_instrument_id := v_existing_id; end if;
  end if;
  ...
  if v_instrument_id is null then
    select count(*), min(id) into v_match_count, v_existing_id
      from public.instruments where lower(trim(name)) = lower(v_name);
    if v_match_count > 1 then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: multiple instruments share name %', v_name;
    end if;
    if v_match_count = 1 then v_instrument_id := v_existing_id; end if;
  end if;

  -- If an incoming identifier belongs to a different existing instrument, never overwrite it.
  if v_isin is not null then
    select id into v_existing_id from public.instruments where isin = v_isin limit 1;
    if v_existing_id is not null and v_instrument_id is not null and v_existing_id <> v_instrument_id then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: ISIN belongs to another instrument';
    end if;
  end if;
  if v_nse_symbol is not null then
    ...
    if v_existing_id is not null and v_instrument_id is not null and v_existing_id <> v_instrument_id then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: NSE symbol belongs to another instrument';
    end if;
  end if;
  if v_bse_code is not null then
    select id into v_existing_id from public.instruments where bse_code = v_bse_code limit 1;
    if v_existing_id is not null and v_instrument_id is not null and v_existing_id <> v_instrument_id then
      raise exception 'INSTRUMENT_IDENTITY_CONFLICT: BSE code belongs to another instrument';
    end if;
  end if;

  if v_instrument_id is null then
    -- Avoid slug collisions without making the slug dependent on import order.
    ...
    insert into public.instruments (
      name, isin, nse_symbol, bse_code, exchange, asset_type, security_type, slug, status, currency
    ) values (
      v_name, v_isin, v_nse_symbol, v_bse_code, v_exchange, v_asset_type, v_security_type, v_slug, 'ACTIVE', 'INR'
  ...
  -- Preserve identifier history. Existing current identifiers for this instrument are reused;
  -- identifiers never get silently reassigned to another instrument.
  if v_isin is not null then
    ...
    on conflict (identifier_type, identifier_value, valid_from) do update
      set instrument_id = excluded.instrument_id, is_current = true, source = excluded.source, updated_at = now();
  end if;
